...
DECLARE @LiveTablesSectionName VARCHAR(50) = 'Live Tables'
	   ...
	   ,@LiveTablesSectionDescription VARCHAR(200) = 'As the name implies, the live tables are where the extracted Confluence data is ultimately stored for use in reporting.'
	   ...
	   ,@LoggingTablesSectionDescription VARCHAR(200) = 'For now, the only logging table is tbl_Confluence_Refresh, which stores audit information about executions of the refresh job.'
...
SELECT   @LiveTablesSectionName
		...
		,[o].[name]
FROM sysobjects o 
WHERE [o].[name] LIKE 'tbl_Confluence_%' AND [o].[name] <> 'tbl_Confluence_Refresh'
...
SELECT   @LoggingTablesSectionName
		...
		,[o].[name]
FROM sysobjects o 
WHERE [o].[name] = 'tbl_Confluence_Refresh'
